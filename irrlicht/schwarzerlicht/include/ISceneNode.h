	...
	*/
	...
	public:
		...
		*/
		...
		//! The internal timer scaling factor allows to animate a node at a different speed.
		/* The value is a floating point multiplier which multiplies the timeMs value in the OnAnimate method,
		propagating to all the children. If a child has a different scaling factor, the multiplication will 
		be performed again when the child's OnAnimate is called. This method accepts only positive values and
		does nothing if the scaling factor is negative.*/
		virtual void SetInternalTimerScalingFactor(f32 scalingFactor) {
			if (scalingFactor >= 0.f) {
				InternalTimerUpdateMultiplier = scalingFactor;
				ISceneNodeList::Iterator it = Children.begin();
				for (; it != Children.end(); ++it)
					(*it)->SetInternalTimerScalingFactor(scalingFactor);
			}
		}
		...
		{
			u32 deltaT = timeMs - lastUpdatedTimeMs;
			lastUpdatedTimeMs = timeMs;
			u32 originalTimeMs = timeMs;
			...
			if (IsVisible)
			{
				...
				while (ait != Animators.end())
					...
					if ( anim->isEnabled() )
					{
						anim->animateNode(this, totalTimeMs);
					}
				}
	...
	protected:
		...
		//! internal timer multiplier
		f32 InternalTimerUpdateMultiplier = 1.f;

		//! initial time (used for creating a suitable update condition)
		u32 totalTimeMs = 0;
		u32 lastUpdatedTimeMs = 0;
	};
