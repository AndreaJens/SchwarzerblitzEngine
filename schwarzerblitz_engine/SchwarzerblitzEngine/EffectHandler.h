...
public:
	...
	*/
	...
	bool nodeHasShadowAttached(irr::scene::ISceneNode* node, E_FILTER_TYPE filterType = EFT_NONE, E_SHADOW_MODE shadowMode = ESM_BOTH);
	...
	/// Add material to buffer for light management
	void addCustomMaterialToBuffer(irr::s32 materialType, irr::s32 shadedMaterial){
		additionalMaterials[materialType] = shadedMaterial;
	}
	...
	*/
	...
	// remove last post-processing effect
	void removeLastPostProcessingEffect(){
		PostProcessingRoutines.erase(PostProcessingRoutines.size()-1);
	}
	...
	/// apply single post processing effect to target
	void applyPostProcessingEffectToTarget(irr::video::ITexture* outputTarget, const irr::core::stringc& filename);
	/// store additional post processing effect for later use
	void storeAdditionalPostProcessingEffect(const irr::core::stringc& effectName, const irr::core::stringc& filenName);
...
private:
	...
	bool useVSM;
	bool DepthPass;
	std::map<irr::core::stringc, irr::video::E_MATERIAL_TYPE> additionalEffects;
	std::map<irr::s32, irr::s32> additionalMaterials;
};
