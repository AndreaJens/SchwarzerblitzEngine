...
bool EffectHandler::nodeHasShadowAttached(irr::scene::ISceneNode *node, E_FILTER_TYPE filterType, E_SHADOW_MODE shadowMode){
	SShadowNode snode = { node, shadowMode, filterType };
	irr::s32 i = ShadowNodeArray.binary_search(snode);
	return i != -1;
}
...
void EffectHandler::update(irr::video::ITexture* outputTarget)
{
	...
	if(!ShadowNodeArray.empty() && !LightList.empty())
	{
		...
		driver->setRenderTarget(ScreenQuad.rt[0], ECBF_COLOR | ECBF_DEPTH, AmbientColour);
		...
		for(u32 l = 0;l < LightListSize;++l)
		{
			...
			driver->setRenderTarget(currentShadowMapTexture, ECBF_COLOR | ECBF_DEPTH, SColor(0xffffffff));
			...
			if(useVSM)
			{
				...
				driver->setRenderTarget(currentSecondaryShadowMap, ECBF_COLOR | ECBF_DEPTH, SColor(0xffffffff));
			...
			driver->setRenderTarget(ScreenQuad.rt[1], ECBF_COLOR | ECBF_DEPTH, SColor(0xffffffff));
			...
			driver->setRenderTarget(ScreenQuad.rt[0], 0, SColor(0x0));
		...
		for(u32 i = 0;i < ShadowNodeArraySize;++i)
		{
			...
			{
				BufferMaterialList.push_back(ShadowNodeArray[i].node->getMaterial(m).MaterialType);
				s32 materialToTest = additionalMaterials.count(BufferMaterialList[m]) > 0 ? 
					additionalMaterials[BufferMaterialList[m]]:
					BufferMaterialList[m];
				switch (materialToTest)
				{
	...
	{
		driver->setRenderTarget(ScreenQuad.rt[0], ECBF_COLOR | ECBF_DEPTH, SColor(0xffffffff));
	}
	
	driver->setRenderTarget(ScreenQuad.rt[1], ECBF_COLOR | ECBF_DEPTH, ClearColour);
	smgr->drawAll();

	const u32 PostProcessingRoutinesSize = PostProcessingRoutines.size();

	driver->setRenderTarget(PostProcessingRoutinesSize 
		? ScreenRTT : outputTarget, ECBF_COLOR | ECBF_DEPTH, SColor(0x0));
	...
	if(DepthPass)
	{
		driver->setRenderTarget(DepthRTT, ECBF_COLOR | ECBF_DEPTH, SColor(0xffffffff));
	...
	if(PostProcessingRoutinesSize)
	{
		...
		for(u32 i = 0;i < PostProcessingRoutinesSize;++i)
		{
			...
			outputTarget : ScreenQuad.rt[int(Alter)], ECBF_COLOR | ECBF_DEPTH, ClearColour);
...
/* apply a single post-processing effect without adding it to the pipeline */
void EffectHandler::applyPostProcessingEffectToTarget(irr::video::ITexture* outputTarget, const irr::core::stringc& filename){
	//const u32 PostProcessingRoutinesSize = PostProcessingRoutines.size();
	ScreenQuad.getMaterial().setTexture(1, ScreenRTT);
	ScreenQuad.getMaterial().setTexture(2, DepthRTT);
	if (additionalEffects.count(filename)){
		ScreenQuad.getMaterial().MaterialType = additionalEffects[filename];
	}
	else{
		additionalEffects[filename] = (E_MATERIAL_TYPE)obtainScreenQuadMaterialFromFile(filename).materialType;
		ScreenQuad.getMaterial().MaterialType = additionalEffects[filename];
	}
	driver->setRenderTarget(outputTarget, ECBF_COLOR | ECBF_DEPTH, ClearColour);
	ScreenQuad.render(driver);
}

/* store additional post processing effects for later use */
void EffectHandler::storeAdditionalPostProcessingEffect(const irr::core::stringc& effectName, const irr::core::stringc& filename){
	if (additionalEffects.count(effectName)){
		additionalEffects[effectName] = (E_MATERIAL_TYPE)obtainScreenQuadMaterialFromFile(filename).materialType;
	}
};
